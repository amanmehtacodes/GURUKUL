...
This story motivates _why_ coordinates are useful: they let us describe a physical space precisely enough that even someone who cannot see the room can understand its layout by "feeling" the marked coordinate positions.
...
A. _(Sample)_ **P(2, 5)** — both coordinates positive, so it's in Quadrant I.

**Q4. Mark any point R in Quadrant III and write its coordinates (example).**
A. _(Sample)_ **R(−4, −3)** — both coordinates negative, so it's in Quadrant III.

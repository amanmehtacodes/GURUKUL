...
export class Program {
  ...
  setShaders({ vertex, fragment }) {
    ...
    for (let uIndex = 0; uIndex < numUniforms; uIndex++) {
      ...
      this.uniformLocations.set(
        uniform,
        this.gl.getUniformLocation(this.program, uniform.name)
      );
  ...
  use({ flipFaces = false } = {}) {
    ...
    this.uniformLocations.forEach((location, activeUniform) => {
      ...
      if (!uniform) {
        return warn(
          `Active uniform ${activeUniform.name} has not been supplied`
        );
      }

      if (uniform && uniform.value === undefined) {
        return warn(
          `${activeUniform.name} uniform is missing a value parameter`
        );
      }

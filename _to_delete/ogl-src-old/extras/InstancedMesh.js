  ...
  addFrustumCull() {
    ...
    if (!this.geometry.attributes.instanceMatrix)
      console.error(
        `mesh ${
          this.name ? `"${this.name}" ` : ``
        }missing instanceMatrix attribute; unable to frustum cull`
      );
    ...
    if (!!this.geometry.attributes.lightmapScaleOffset) {
      ...
      for (let i = 0, j = 0; i < lightmapData.length; i += 4, j++) {
        this.instanceTransforms[j].lightmapData = new Vec4().fromArray(
          lightmapData,
          i
        );
      }
    ...
    this.frustumCullFunction = ({ camera }) => {
      ...
      this.instanceRenderList.forEach((transform, i) => {
        ...
        if (transform.lightmapData) {
          transform.lightmapData.toArray(
            this.geometry.attributes.lightmapScaleOffset.data,
            i * 4
          );

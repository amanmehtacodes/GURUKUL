...
export class GLTFLoader {
  ...
  static async parse(gl, desc, dir) {
    ...
    if (
      desc.extensionsRequired?.includes("KHR_draco_mesh_compression") &&
      !this.dracoManager
    )
      console.warn(
        "KHR_draco_mesh_compression extension required but no manager supplied. Use .setDracoManager()"
      );

    if (
      desc.extensionsRequired?.includes("KHR_texture_basisu") &&
      !this.basisManager
    )
      console.warn(
    ...
    const meshes = await this.parseMeshes(
      gl,
      desc,
      bufferViews,
      materials,
  ...
  static parseBufferViews(gl, desc, buffers) {
          ...
          for (const attr in attributes) {
            ...
            if (accessor.bufferView === undefined && !!extensions) {
              ...
              if (extensions.KHR_draco_mesh_compression) {
                accessor.bufferView =
                  extensions.KHR_draco_mesh_compression.bufferView;
    ...
    bufferViews.forEach(
      (
        ...
        bufferViews[i].data = buffers[bufferIndex].slice(
          byteOffset,
  ...
  static parseImages(gl, desc, dir, bufferViews) {
    ...
    return Promise.all(
      desc.images.map(
        async ({ uri, bufferView: bufferViewIndex, mimeType, name }) => {
  ...
  static parseTextures(gl, desc, images) {
    ...
    return desc.textures.map((textureInfo) =>
      this.createTexture(gl, desc, images, textureInfo)
    );
  }
        ...
        async (
          ...
          if (isSkin) {
            ...
            primitives = (
              ...
              const meshConstructor = geometry.attributes.instanceMatrix
                ? InstancedMesh
                : Mesh;
        ...
        async ({
          ...
          if (extensions && extensions.KHR_draco_mesh_compression) {
            ...
            const gltfAttributeMap =
              extensions.KHR_draco_mesh_compression.attributes;
            const attributeMap = {};
            ...
            for (const attr in attributes) {
              ...
              attributeTypeNameMap[attributeName] =
                TYPE_ARRAY[accessor.componentType].name;
    ...
    const nodes = desc.nodes.map(
      ({
        ...
        if (extras && extras.lightmapTexture !== undefined) {
          extras.lightmapTexture.texture = this.createTexture(
            gl,
            desc,
            images,
      ...
      (
        ...
        const data = channels.map(
          ({
            ...
            const times = this.parseAccessor(
              inputIndex,
              desc,
              bufferViews
            ).data;
  ...
  static parseLights(gl, desc, nodes, scenes) {
    ...
    scenes.forEach((scene) =>
      scene.forEach((node) => node.updateMatrixWorld())
    );

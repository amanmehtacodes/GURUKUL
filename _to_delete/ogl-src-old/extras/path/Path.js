...
import {
  ...
  toDegrees,
  toRadian,
  mat4fromRotationSinCos,
  rotateNormalBinormal,
} from "./utils.js";
...
export class Path {
  constructor() {
    ...
    this._assertLastPoint = throwIfNullProperty.bind(
      this,
      "_lastPoint",
      "Can`t get previous point of curve. Did you forget moveTo command?"
    );
  ...
  bezierCurveTo(cp1, cp2, p, tilt = 0) {
    ...
    const seg = new CubicBezierSegment(
      this._lastPoint,
      cp1,
      cp2,
      p,
  ...
  quadraticCurveTo(cp, p, tilt = 0) {
    this._assertLastPoint();
    const seg = new QuadraticBezierSegment(
      this._lastPoint,
      cp,
      p,
      this._lastTilt,
  ...
  computeFrenetFrames(divisions = 64, closed = false) {
    ...
    const totalLength = this.getLength();
    for (let i = 0; i <= divisions; i++) {
      const [si, st] = this.findSegmentIndexAtLength(
        (totalLength * i) / divisions
      );
    ...
    if (closed === true) {
      ...
      let step =
        Math.acos(clamp(normals[0].dot(normalLast), -1, 1)) /
        (normals.length - 1);
